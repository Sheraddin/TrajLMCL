...
class Data:
    def __init__(self, name, road_type='road_network', **kwargs):
        ...
        # Determine paths based on name (handle D0-D4 suffixes for path lookup)
        base_name = name
        if "_D" in name and name.split("_D")[-1].isdigit():
             # If name is small_chengdu_D0, we likely want to look in the same folder as small_chengdu
             pass
    ...
    def read_hdf(self):
        ...
        print(f"Loading DataFrame from {self.df_path}...")
        ...
        # --- FIX: Ensure time column is datetime ---
        ...
        # Standard split 80/10/10 for the loaded dataset
    ...
    def generate_cl_splits(self):
        """
        Splits the current dataset into D0, D1, D2, D3, D4 based on time.
        D0: 30%, D1-D4: 17.5% each.
        """
        print(f"Generating CL splits for {self.name}...")
        
        # 1. Get start time for each trip to sort them chronologically
        print("Sorting trips by time...")
        # Assuming 'seconds' is already computed in read_hdf, or we compute it from 'time'
        if 'seconds' not in self.trips.columns:
             self.trips['seconds'] = self.trips['time'].apply(lambda x: x.timestamp())
        
        trip_start_times = self.trips.groupby('trip')['seconds'].min()
        sorted_trips = trip_start_times.sort_values().index.to_numpy()
        
        total_trips = len(sorted_trips)
        print(f"Total unique trips: {total_trips}")

        # 2. Define split ratios and indices
        ratios = [0.30, 0.175, 0.175, 0.175, 0.175]
        # Verify sum is close to 1
        assert abs(sum(ratios) - 1.0) < 1e-5, f"Ratios sum to {sum(ratios)}, expected 1.0"

        idxs = [0]
        current_idx = 0
        for r in ratios:
            current_idx += int(r * total_trips)
            idxs.append(current_idx)
        # Ensure last index covers everything
        idxs[-1] = total_trips
        
        print(f"Split indices: {idxs}")

        # 3. Create splits
        # Load POIs once to copy them
        try:
            pois_df = pd.read_hdf(self.df_path, key='pois')
        except KeyError:
            print("Warning: 'pois' key not found in source HDF5.")
            pois_df = None

        for i in range(5):
            split_name = f"{self.name}_D{i}"
            output_path = f"{self.dataset_path}/{split_name}.h5"
            
            start_i = idxs[i]
            end_i = idxs[i+1]
            split_trip_ids = sorted_trips[start_i:end_i]
            
            print(f" Creating {split_name} ({len(split_trip_ids)} trips)...")
            
            # Filter DataFrames
            split_trips_df = self.trips[self.trips['trip'].isin(split_trip_ids)].copy()
            split_info_df = self.trip_info[self.trip_info['trip'].isin(split_trip_ids)].copy()
            
            # Save to HDF5
            split_trips_df.to_hdf(output_path, key='trips', mode='w')
            split_info_df.to_hdf(output_path, key='trip_info', mode='a')
            self.road_info.to_hdf(output_path, key='road_info', mode='a')
            if pois_df is not None:
                pois_df.to_hdf(output_path, key='pois', mode='a')
...
if __name__ == '__main__':
    ...
    # NEW ARGUMENT FOR SPLITTING
    parser.add_argument('--split_cl', action='store_true', help="Split dataset into D0-D4 for Continual Learning")
    ...
    # EXECUTE SPLITTING IF REQUESTED
    if args.split_cl:
        data.generate_cl_splits()
        print("Splitting complete. Now run preprocessing for each split.")
        exit()
